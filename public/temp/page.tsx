...
import { CheckCircle, Download, Mail, Phone, Calendar, Gift } from 'lucide-react';
...
function CheckoutSuccessPageContent() {
  ...
  useEffect(() => {
    console.log('[SUCCESS PAGE] 🔄 Component mounted/updated', {
    ...
    if (orderId) {
      ...
      console.log('[SUCCESS PAGE] ⚠️ No orderId in URL');
      setLoading(false);
    }
  }, [orderId]);

  const loadOrderDetails = async (orderIdParam: string) => {
    console.log('[SUCCESS PAGE] 🎉 Loading order details', {
      timestamp: new Date().toISOString(),
    ...
    try {
      ...
      console.log('[SUCCESS PAGE] 📦 Order details received', {
        success: data.success,
        orderNumber: data.data?.orderNumber,
        itemCount: data.data?.orderItems?.length,
        giftCardItems: data.data?.orderItems?.filter((item: any) => item.type === 'GIFT_CARD').length,
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-start gap-2 mb-2">
                        <h4 className="font-semibold text-lg flex-1">{getItemTitle(item)}</h4>
                    ...
                    <div className="text-right">
                      ...
                      {item.type === 'GIFT_VOUCHER' ? (
                        <div>
                          ...
                          <p className="font-bold text-lg text-cyan-600">GRATUIT</p>
                        </div>
                      ) : (
          ...
          <CardContent className="space-y-4">
                      ...
                      return (
                        <div key={item.id} className="flex justify-between text-sm">
                          <span className="text-cyan-700">
                            🎁 Baptême {item.participantData.selectedCategory} - {item.participantData.firstName} {item.participantData.lastName}
                            <span className="text-xs text-cyan-600 block">
                              {formatDate(item.bapteme?.date)} • Bon cadeau appliqué
                            </span>
                          </span>
              ...
              {order.discountAmount > 0 && (
                <div className="mt-3 pt-3 border-t border-green-300 bg-green-100 rounded p-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-green-800 font-medium">Cartes cadeaux appliquées</span>
            ...
            {calculateTotals().remainingTotal > 0 && (
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                ...
                <p className='text-xs text-orange-700 mb-3'>Les soldes seront à régler sur place le jour de l'activité.</p>
                  ...
                  {calculateTotals().futurePayments.map((payment, index) => (
                    <div key={index} className="text-sm border-l-2 border-orange-300 pl-3">
                      ...
                      <p className="text-xs text-orange-600 mt-1">
                        À régler <span className='font-semibold underline'>sur place</span> le jour de l'activité
                      </p>
                    </div>
                  ))}
                </div>
