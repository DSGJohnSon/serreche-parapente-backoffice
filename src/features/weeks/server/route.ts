...
import { CreateWeekSchema, UpdateWeekSchema } from "../schemas";
import { z } from "zod";
...
const app = new Hono()
  ...
  .post(
    "create",
    zValidator("json", CreateWeekSchema),
    sessionMiddleware,
    async (c) => {
      try {
  ...
  .post(
    "update",
    zValidator("json", UpdateWeekSchema),
    sessionMiddleware,
    async (c) => {
      try {
        ...
        const previousData = await prisma.week.findUnique({
          where: {
            startDate: startDateObj,
          },
          include: {
